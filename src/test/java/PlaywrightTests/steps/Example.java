...
//            });
        try (Playwright playwright = Playwright.create()) {
            ...
            Page page = context.newPage();
            page.navigate("https://service-test.nci.nih.gov/nih-ssj.do#/");
            page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName("DIEGO TEST 20240820145734")).click();
            page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Sign In and Apply")).click();
            Page page1 = page.waitForPopup(() -> {
                page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Log in")).click();
            });
            assertThat(page1.locator("#form19")).containsText("Sign in with PIV / CAC card");
        }
        }
    }