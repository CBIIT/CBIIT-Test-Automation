Feature: Create OA Intake form
  ...
  Scenario: Create OA Intake form
    ...
    And Submitter User can verify that they are on an OA Intake Request page
    And Submitter User can verify Important paragraph text
    ...
    And Submitter User clicks on New Request tab
    And Submitter User confirms that Status is "Draft" by default
    And Submitter User confirms that Status Draft is a disabled field by default
    ...
    Then Submitter User can see alert message "Task Order Acquisition Plans (TOAPs) are required for both Internal and External Call/ Order submissions."
    And Submitter User chooses Internal NCI Call/Task Order option on a BPA/IDIQ as Yes
    ...
    And Submitter User chooses FITARA/Business Case Clearances option as No
    Then Submitter User can enter "100000" for Please enter the estimated value of IT and/or biomedical computing requirement
