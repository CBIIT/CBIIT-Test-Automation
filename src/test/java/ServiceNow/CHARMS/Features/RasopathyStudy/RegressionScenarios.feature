Feature: MyRAS Study Regression Scenarios
  ...
  @juarezds @e2e-Regression @Updated
  Scenario: Proxy Screener Submission with data verification
    Given a proxy is on the RASopathies Longitudinal Cohort Study login page
    ...
    And clicks next after reviewing the STUDY INTRODCTION
    And selects I am completing this form for someone else option
    And enters First Name "AutomatedFirstNameOfParticipant", Middle Name "Middle", Last Name "AutomatedLastNameOfParticipant" for What is the name of the person who may be eligible for this study?
    And enters First Name "FirstNameOfProxyTest", Middle Name "MiddleNameTest", Last Name "lastNameOfProxyTest" for What is your name?
    And enters "Sending 100 characters as a string as a test Sending 100 characters as a string as a test Sending 100" for What is your relationship to the participant?
    And selects Yes option for Are you the legal guardian of the participant?
    And clicks next for The next set of questions will collect basic information about the participant
    And selects participants Date of Birth from the calendar picker
    And selects Male option for participants Sex assigned at birth
    And selects Yes option for participant being adopted
    And selects Yes for participant still being alive
    And selects country "United States of America" for In which country does participant currently live?
    And selects Yes option for participant currently living in the United States
    And enters mailing address for where study materials can be sent, street "9609 Medical Center Dr" street Optional "New Avenue Park" city "Rockville" state "MD" zip code "20850"
    And enters proxy email address "automatedTest@nci.gov"
    And enters proxy email address "automatedTest@nci.gov" to confirm email address
    And enters proxy home phone number "703-687-5816", cell phone number "703-687-5816", work phone number "703-687-5816" and selects cell phone number as Preferred Contact
    And selects Not Hispanic/Latino option for Ethnicity of participant
    And selects White, Black/African American, Native Hawaiian/Pacific Islander, American Indian/Alaskan Native, Asian and enters "Sending 100 characters as a string as a test Sending 100 characters as a string as a test Sending 100" for other when selecting Race of participant
  ...
  # And logs in via Okta with username "charmsras1@yahoo.com" and password "RASTest2021$$"
  # And clicks on the Individual Information Questionnaire to begin the IIQ form
  # And enters email address "charmsras1@yahoo.com" and One Time Pin code to proceed with the form
  ...
  # And logs in via Okta with username "charmsras1@yahoo.com" and password "RASTest2021$$"
  # And clicks on the RAS Survey to begin the RAS Survey
  # And enters Okta email address "charmsras1@yahoo.com" and One Time Pin code to proceed with the RAS Survey
  # And clicks on survey next button after reviewing the survey details to start on the myRAS Survey questions. page - myRAS Survey
  # And clicks next button on The first block of questions will collect basic demographic information. page - myRAS Survey
