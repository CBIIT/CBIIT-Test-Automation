...
public class NativeViewGroupingsVerificationSteps extends PageInitializer {
	...
	public void navigating_to_the_Specimens_module_in_Native_View() throws TestingException {
		...
		MiscUtils.sleep(1000);
		JavascriptUtils.selectDateByJS(nativeViewEnrollmentViewPage.nativeViewFollowUpParticipantEventsConsentedDate, "01/20/2021");
		MiscUtils.sleep(3000);
	...
	public void the_following_fields_display(String wereYouSymptomatic, String howWasTheSampleTaken, String typeOfTest, String dateOfficiallyDiagnosed) {
	    Assert.assertTrue(nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyWereYouSymptomaticText.getText().contentEquals(wereYouSymptomatic));
	    Assert.assertTrue(nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyHowWasTheSampleTakenText.getText().contentEquals(howWasTheSampleTaken));
	    Assert.assertTrue(nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyWhatTypeOfTestDidThePatientReceiveText.getText().contentEquals(typeOfTest));
	    Assert.assertTrue(nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyHaveYouOfficiallyBeenDiagnosedText.getText().contentEquals(dateOfficiallyDiagnosed));
	    MiscUtils.sleep(2000);
		CucumberLogUtils.logScreenShot();
	}
	...
	public void when_selecting_to_Were_you_symptomatic(String yes) {
		 CommonUtils.selectDropDownValue(yes, nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyWereYouSymptomaticDD);
		 MiscUtils.sleep(2000);
	     CucumberLogUtils.logScreenShot();
	}
	...
	public void when_did_you_first_develop_symptoms_of_COVID_date_picker_displays(Integer int1) {
	  Assert.assertTrue(nativeViewEnrollmentViewPage.nativeViewFollowUpSymptomologyWhenDidYouFirstDevelopSymptomsTextLabel.getText().contentEquals("When did you first develop symptoms of COVID-19?"));
	  MiscUtils.sleep(2000);
	  CucumberLogUtils.logScreenShot();
	}
