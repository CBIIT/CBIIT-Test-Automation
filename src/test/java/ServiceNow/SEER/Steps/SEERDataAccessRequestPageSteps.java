...
public class SEERDataAccessRequestPageSteps extends PageInitializer {
	...
	public void user_verifies_email_address_by_clicking_on_the_email_verification_link_that_is_sent_to_the_user_after_submitting_a_SEER_User_registration_form() throws TestingException{
		...
		Assert.assertTrue(nativeViewSentViewPage.nativeViewPreviewEmailHelloNameText.getText().contains("TestFIRSTname"));
		MiscUtils.sleep(2000);
	...
	public void the_user_is_directed_to_the_page(String pageTitle) {
		String landingPage = WebDriverUtils.getWebDriver().getTitle();
		Assert.assertTrue(landingPage.contains(pageTitle));
		MiscUtils.sleep(2000);
	}

	@Then("the {string} field contains the name of the requestor in read-only format")
	public void the_field_contains_the_name_of_the_requestor_in_read_only_format(String string) {
