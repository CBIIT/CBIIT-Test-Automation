Feature: Top Accomplishments Submission
  ...
  @NERDSNOW-759 @Progression @Smoke @playwright @In-Progress
...
#    And subtext of "Please select a specific cancer site only if the submission is focused on the specific site" when hover over the Cancer Site/Type field
...
#    And 'References' is an optional short text field THAT converts the input plain text into a URL hyperlink after submission (Ex. Users submit a plain text PMID from PubMed (https://pubmed.ncbi.nlm.nih.gov/39282897/)
#    And subtext of "Please list PMIDs only, separated by commas" when hover over the References field
#    And 'Other References' is an optional short text field THAT converts the input plain text into a URL hyperlink after submission
...
#    And 'Point of Contact' is an optional list collector field AND offers multi-selection so more than one Point of Contact can be selected AND pulls all available users from the global NED users table
#    And 'Point of Contact Email' is an optional field THAT gets populated with the email of the user selected from the 'Point of Contact' field AND is empty if no user is selected as a 'Point of Contact' once form is saved or submitted