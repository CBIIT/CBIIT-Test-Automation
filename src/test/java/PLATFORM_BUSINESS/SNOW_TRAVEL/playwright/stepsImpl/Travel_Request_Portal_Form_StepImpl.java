...
public class Travel_Request_Portal_Form_StepImpl {
    ...
    public static void the_field_will_map_to_the_travel_start_date_field_in_the_variables_section_of_the_ritm_in_nv(String travelStartDate) {
        ...
        page.frameLocator(iframeLocator).locator("(//*[@class='list_decoration_cell col-small col-center ']//a)[1]").hover();
        page.frameLocator(iframeLocator).locator("(//*[@class='list_decoration_cell col-small col-center ']//a)[1]").click();
        page.frameLocator(iframeLocator).locator("//*[@class='btn btn-sm btn-default pop-over-button pull-right']").click();
        CucumberLogUtils.playwrightScreenshot(page);
        page.frameLocator(iframeLocator).locator("#tabs2_section").getByText(Travel_Request_Portal_Form_Constants.VARIABLES_TAB).click();
        page.frameLocator(iframeLocator).locator("//span[@aria-label='Trip Information']").scrollIntoViewIfNeeded();
        assertThat(page.frameLocator(iframeLocator).getByLabel("Trip Information", new FrameLocator.GetByLabelOptions().setExact(true))).containsText(Travel_Request_Portal_Form_Constants.TRIP_INFORMATION_HEADER_TEXT);
        assertThat(page.frameLocator(iframeLocator).getByRole(AriaRole.HEADING, new FrameLocator.GetByRoleOptions().setName(travelStartDate))).isVisible();
        assertThat(page.frameLocator(iframeLocator).getByLabel("Variables form section")).containsText(travelStartDate);
        CucumberLogUtils.playwrightScreenshot(page);
    }
    ...
    public static void the_field_will_map_to_the_travel_end_date_in_the_variables_section_of_the_ritm_in_nv(String travelEndDate) {
        assertThat(page.frameLocator(iframeLocator).getByRole(AriaRole.HEADING, new FrameLocator.GetByRoleOptions().setName(travelEndDate))).isVisible();
        assertThat(page.frameLocator(iframeLocator).getByLabel("Variables form section")).containsText(travelEndDate);
        CucumberLogUtils.playwrightScreenshot(page);
    }
